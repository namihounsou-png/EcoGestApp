        ...
        <form onSubmit={handleSubmit} className="max-w-2xl mx-auto space-y-6">
          {error && (
            <div className="bg-red-100 border border-red-400 text-red-700 px-4 py-3 rounded relative" role="alert">
              <strong className="font-bold">Erreur: </strong><span className="block sm:inline">{error}</span>
            </div>
          )}
          ...
          <Card>
            ...
            <div
              ...
              className="relative h-64 bg-secondary-100 rounded-lg flex items-center justify-center cursor-pointer overflow-hidden"
            >
              {photo ? (
                <motion.div
                  initial={{ scale: 0.95 }}
                ...
                >
                  <Image src={photo} alt="Aperçu" fill className="object-cover" />
                </motion.div>
              ) : (
              ...
              {(['low','medium','high'] as UrgencyLevel[]).map(level => (
                <motion.button
                  ...
                  className={`px-4 py-2 rounded-full border text-xs font-medium transition-colors ${
                    urgency === level ? urgencyColors[level] : 'border-secondary-300 text-secondary-400'
                  }`}
                >
