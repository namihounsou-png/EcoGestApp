        ...
        <form onSubmit={handleSubmit} className="max-w-2xl mx-auto space-y-6">
          ...
          <Card className="p-6">
            ...
            <div onClick={() => fileInputRef.current?.click()} className="relative h-64 bg-secondary-200 rounded-lg flex items-center justify-center cursor-pointer overflow-hidden">
              ...
              <div className="absolute inset-0 bg-black/50 flex items-center justify-center text-white opacity-0 hover:opacity-100 transition-opacity">
                <Upload className="w-8 h-8 mr-2" /> Changer l'image
              </div>
            </div>
          </Card>
          ...
          <Card className="p-6">
            ...
            <div className="flex justify-around">
              <button type="button" onClick={() => setUrgency('low')} className={`px-4 py-2 rounded-lg text-sm font-semibold transition-all ${urgency === 'low' ? 'bg-primary-500 text-white' : 'bg-secondary-100'}`}>Faible</button>
              <button type="button" onClick={() => setUrgency('medium')} className={`px-4 py-2 rounded-lg text-sm font-semibold transition-all ${urgency === 'medium' ? 'bg-accent-500 text-white' : 'bg-secondary-100'}`}>Moyenne</button>
              <button type="button" onClick={() => setUrgency('high')} className={`px-4 py-2 rounded-lg text-sm font-semibold transition-all ${urgency === 'high' ? 'bg-red-500 text-white' : 'bg-secondary-100'}`}>Haute</button>
            </div>
          </Card>
          ...
          <div className="sticky bottom-0 py-4 bg-white/80 backdrop-blur-sm border-t">
